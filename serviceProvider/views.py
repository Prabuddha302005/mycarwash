...
def home(request):
    ...
    if request.user.is_staff:
        print(f"Logged in user: {request.user.username}, ID: {request.user.id}, is_staff: {request.user.is_staff}")
        
        provider_services = Services.objects.filter(sid=request.user.id)
        print(f"Provider services: {provider_services}")
        
        get_bookings = Bookings.objects.filter(service__in=provider_services)
        print(f"Bookings: {get_bookings}")
        user_ids = get_bookings.values_list('user_id', flat=True).distinct()
        get_address = Address.objects.filter(user_id__in=user_ids)
        data['bookings'] = get_bookings
        data['useraddress'] = get_address
...
def add_details(request):
    ...
    if(not Business_details.objects.filter(id=user_id)):
        ...
        return render(request, "serviceProvider/already.html")
